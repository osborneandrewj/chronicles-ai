...
- **Phase 2 — archetypes: ⏳ partial.** `stub-crew-generator.ts` now honors `template.eraTags` (was hardcoded `['sci-fi','space','generic']`), so offline/test crews are era-appropriate. **Deferred (lower leverage now):** genre-filtering `pickHubArchetype` + new non-containment archetypes only matter for the **simulation/bounded** paths (SIM_HUB is OFF in prod and grounded adventures no longer draw hubs).
- **Phase 3 — vocab regexes: ⛔ blocked on a prerequisite.** Recon correction: the live code is `domain/services/narrator-guidance.ts` (the audit's `lib/...` path is a stale **re-export shim**). The six predicates are **heavily tuned/overfit** (prod-world-12) — any change must default to the current arrays (byte-identical) and is a **narrator change requiring a streamed-turn re-test**. They run for *all* worlds (incl. grounded), so this is the highest remaining value — but per-world term lists need a **genre signal on `World`**, which does not exist yet ⇒ depends on Phase 4.
- **Phase 4 — schema: ⏳ not started.** Next free SQLite migration is **v33**. Adding a genre/`meta_frame_kind` field is dual-store: SQLite `migrations.ts` (runs on boot) **and** the Mongo model + mapper (**prod is Mongo Atlas** — use an optional defaulted field so existing docs are safe).
- **Phase 5 — renames: ⏳ not started** (opportunistic, lowest leverage).
