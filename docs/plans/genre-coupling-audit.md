...
## Implementation progress (updated 2026-06-14, branch `feat/genre-decoupling`)
...
- **Phase 2 — archetypes: ✅ DONE.** `stub-crew-generator.ts` honors `template.eraTags`. `WorldArchetype` gained `genres`; the 4 existing hubs are tagged and **5 new period hubs** added (`feudal-village`, `castle-keep`, `ziggurat-temple`, `royal-court`, `caravanserai`) — each a connected 5-room plan with a simulation room, entry, and 4 resident slots (authored in parallel; validated by the existing topology test). Pure `filterHubsByGenre(hubs, eraTags)` keeps era-matching hubs (falling back to the full pool) and is wired at the simulation hub-pick, so a medieval simulation draws a village/castle/monastery, never a starship. +4 tests.
- **Phase 3 — vocab regexes: ✅ every cross-genre BREAK closed (additive, byte-identical).** Recon correction applied: edited the **live** `domain/services/narrator-guidance.ts` (the audit's `lib/...` path is a stale re-export shim). Broadened all six move detectors and `npc-promotion.isTransientServiceNpc` to be **genre-inclusive — purely additive**, so existing modern/sci-fi worlds stay byte-identical (no characterization test moved) while period actions now fire. Added a `temple` occupancy profile + period `KIND_ALIASES`, and era-gated the traffic prose (see Phase 4). **Still a narrator-guidance change → needs a streamed-turn re-test before deploy.** Deferred: `name-resolution` `GENERIC_ROOM_KEYS` (feeds a deterministic place-MERGE; period room words risk new mis-merges without scenario coverage).
- **Phase 4 — genre field on `World`: ✅ DONE (the schema *generalization* is intentionally not built).** Migration **v33** `world_genre` adds `worlds.genre_tags` (nullable JSON), mirrored dual-store (Mongo `WorldDoc.genreTags` + schema default + mapper). `WorldRepository.setGenreTags` on the port + both adapters; captured at creation (grounded adventures from the preset's era+tone tags, quick-start from its genre). First consumer: era-gated occupancy traffic (`eraFromGenreTags`/`parseGenreTags` + a `TrafficEra` param threaded from `narrate-turn`) — a premodern world reads "carts and pack animals", not "idling engines"; modern/genre-less worlds stay byte-identical. **SQLite 706 + Mongo 27 green.** The `world_session`/widened-`WorldLayer` generalization (for non-simulation **multi-layer** stories) is **NOT built** — no such story shape exists now that grounded adventures are single-layer standalone, so building it would be speculative.
- **Phase 5 — cosmetic renames: ⏳ intentionally not done as a dedicated pass.** The audit itself scopes Phase 5 as "do opportunistically when touching the file, **not as a dedicated effort**." Its items are either **column renames** (`ship_clock_minutes`, `places.deck`, `npc_reveries` → migrations with prod-Mongo risk and zero behavior change) or **prompt-framing edits** (`time-passage.md`, `ensemble-*.md` → change LLM output, want a smoke test) or pure **symbol renames** (`DeckGraph`→`LocationGraph`, 18 sites, zero behavior change). Doing any as a dedicated churn pass contradicts the audit's own guidance and AGENTS.md "simplicity first / nothing speculative." Left for opportunistic pickup.

**The throughline:** every actual cross-genre *break* and *degradation* is now fixed and test-verified across **Phases 1–4** (meta-frame opt-in, era-keyed crews, genre-filtered + expanded archetypes, genre-inclusive vocab, the genre field + era-gated traffic). What's intentionally NOT built is **speculative** (the `world_session` multi-layer generalization, with no story shape needing it) or **opportunistic-by-the-audit's-own-framing** (Phase 5 renames). **Verification gates a human must clear before *deploy*:** a **streamed narrator turn** on a non-sci-fi genre (AGENTS.md "done" for the Phase-3 guidance change) and **prod-Mongo confirmation** of the v33 additive column (default-safe, but confirm on deploy). All landed work passes `npm test` (**706 green**) + `npm run test:mongo` (**27 green**) + depcruise.

### What remains (intentionally not built this pass)

Phases 1–4 are implemented and test-verified. The residue, with the reason it was left:

1. **Pre-deploy verification (human-in-the-loop):** stream a narrator turn on ≥1 non-sci-fi genre (the Phase-3 guidance change is additive/conservative but the predicates were tuned against prod world 12); confirm the v33 `genre_tags` column on prod Mongo (additive + default-null, so existing Atlas docs are safe — just verify on deploy).
2. **`name-resolution` `GENERIC_ROOM_KEYS` period words** — feed a deterministic place-MERGE; adding period room words risks new mis-merges in existing worlds without scenario coverage. Needs targeted merge-scenario tests first.
3. **The `world_session` / widened-`WorldLayer` generalization** — supports non-simulation *multi-layer* stories that don't exist (grounded adventures are single-layer standalone). Speculative until such a story shape is designed.
4. **Per-genre vocab *suppression*** (not firing "phone" in 50 BC) — a refinement on top of the now-additive baseline; the `genre_tags` field is in place to enable it, but its value is marginal and it changes narrator behavior (re-test).
5. **Phase 5 cosmetic renames** — per the audit's own "opportunistic, not a dedicated effort" framing; column renames are migrations, prompt-framing wants a smoke test.
...
- **Mostly data/generalization work.** A new archetype isn't *done* (per AGENTS.md) until a real seed call produces era-appropriate rooms, names, and ensemble.
...
- **Requires a DB migration in `migrations.ts`** that applies cleanly on boot, with the matching Mongo model/index under `infrastructure/persistence/mongo/models` updated and queries still typechecking (per AGENTS.md "done" rules). Sequence *after* Phase 1 so the application code already reads the new shape.
