...
- **Respect the budget.** History packing prefers full narrator turns within a ~4.2k-token full-content window; system prompt, authoritative STATE, and player action are pinned. Soft craft guidance is sparse/risk-gated — do not reintroduce always-on length bands or invent a hard `maxOutputTokens` without a plan. Don't dump full history.
