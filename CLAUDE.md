...
The header at `src/app/page.tsx:17` reads `pkg.version` from `package.json` — that one number is the user's only at-a-glance trust signal for "what's running". Treat it as load-bearing. Full playbook in `docs/RELEASING.md`; the binding rules:
...
- Mechanically: prefer `npm version minor` / `npm version patch`, but ALWAYS verify the lockfile updated in BOTH spots (top-level `"version"` and the entry under `"packages": { "": { ... } }`) — `npm version` may not touch the lock reliably.
