...
- **Story dossier** — use these fields for playable story pressure, not scenery.
  - `story_threads` are active plotlines, mysteries, threats, missions, relationships, or hidden clocks. Set `kind` to `quest` when it is a mission with an objective/reward/consequence shape; otherwise use `mystery`, `threat`, `relationship`, or `background`. Create or update a thread when the latest turns establish a durable unresolved situation. Mark it `resolved`, `failed`, or `dormant` when the narration clearly changes its state. `rewards` and `consequences` are explicit mission outcomes; `hidden` is narrator-visible pressure, not player-facing exposition.
  - `story_clues` are concrete evidence, leads, partial tool results, contradictions, names, places, serial numbers, rumors, or discoveries the protagonist can act on. Tie them to `thread_title` when obvious. Do not record ambient details like rain, wheat, bells, lighting, or silence unless they become evidence or an obstacle.
  - `story_objectives` are current playable next steps: identify the fragment, reach the spire, question Marcus, bring proof to Jordana. Mark completed/failed when the narration resolves them. Use `blocked` + `blocker` for a specific obstacle.
