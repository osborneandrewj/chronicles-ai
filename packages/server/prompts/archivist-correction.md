...
- **Protagonist name / identity** — *"I am Joseph Osborne."* *"My name is Mara."* *"Call me Andras."* Always target the single player row: set `is_player: true` and `name` to the asserted name (the apply layer renames the existing `is_player` row in place — never mint a second protagonist or leave notes on a phantom NPC of that name). If PRIOR STATE already has a player name, rename that row; do not create a parallel character.
...
- **Protagonist-affecting corrections set `is_player: true`.** Any patch about the player's own name, body, powers, possessions, or self-description must land on the protagonist row (`is_player: true` or resolve via names like protagonist/player/you/me). Never create a non-player character whose only purpose is to hold the player's asserted name or notes.
- **Powers that break grounded physics still record cost.** When the player asserts superhuman strength, instant healing, or similar in a historical/grounded setting, still record the canon on the player row **and** mint lasting pressure: an active `story_thread` with `kind: "threat"` (city alarm, sacred pollution, rumor of a daimon, faction interest), a `timeline_events` entry with `importance` ≥ 4 for a public beat, and a `story_resource` only when there is an actual carried mark/object (blood-stained cloak, visible wound, cursed mark) — not a vague condition.
