...
- **speech_hint on talk-shaped plans.** When the plan involves speech (confront, warn, recruit, question, withhold, or the planned_action implies talking), emit a short `speech_hint`: staging edge only — interrupt? one question? withhold? formality shift? silence as the move? Example: `cuts him off; one hard question; no softener`. Never write the full line the character will say. Cap ~160 chars.
