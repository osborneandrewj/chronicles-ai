// DeckPlanProvider (starship P0) — supplies authored deck-plan templates for
...
// member's daily_loop should anchor (must reference a real DeckPlanRoom.key).
export type DeckPlanCrewSlot = {
  role: string
  homeRoomKey: string
  description: string
}
...
export type DeckPlanTemplate = {
  id: string
  name: string
  rooms: DeckPlanRoom[]
  edges: DeckPlanEdge[]
  crew: DeckPlanCrewSlot[]
}
