import type { PlaceProfileRow, PopulationTemplateRow, StoryThread } from '@/domain/entities'

// ---------------------------------------------------------------------------
// Pure occupancy simulation (P4 extraction from lib/place-population.ts). The
// orchestrator `buildPlaceOccupancySnapshot` (which reads/writes the DB around
// this core) stays in lib/place-population.ts until repositories land (P5).
//
...
export function profileFromRow(row: PlaceProfileRow): InferredProfile {
  return {
