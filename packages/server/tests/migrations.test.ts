...
expect(db.pragma('user_version', { simple: true })).toBe(41)
    ...
    // v30: the tracked-object ledger columns on story_resources.
    const resourceColumns = db
      .prepare("PRAGMA table_info('story_resources')")
      .all() as Array<{ name: string }>
...
describe('v35 migration (world_corrections_hidden)', () => {
  it('adds a NOT NULL hidden flag to world_corrections defaulting to 0', () => {
    const db = seedV4Database()
    db.prepare(
      `INSERT INTO worlds (id, name, premise, initial_state_json) VALUES (?, ?, ?, ?)`,
    ).run(1, 'Test World', 'p', JSON.stringify({ time: 't', location: 'l', identity: 'i' }))

    runMigrations(db)

    const cols = db.prepare("PRAGMA table_info('world_corrections')").all() as Array<{
      name: string
      type: string
      notnull: number
      dflt_value: string | null
    ...
    expect(hidden?.type.toUpperCase()).toBe('INTEGER')
    expect(hidden?.notnull).toBe(1)
    expect(hidden?.dflt_value).toBe('0')
  })

  it('withholds hidden rows from the scrollback read without deleting them', () => {
    const db = seedV4Database()
    db.prepare(
      `INSERT INTO worlds (id, name, premise, initial_state_json) VALUES (?, ?, ?, ?)`,
    ).run(1, 'Test World', 'p', JSON.stringify({ time: 't', location: 'l', identity: 'i' }))
    ...
    insert.run(1, 'visible one', 'ok', '{}')
    insert.run(1, 'hidden one', 'ok', '{}')
    db.prepare(`UPDATE world_corrections SET hidden = 1 WHERE player_text = 'hidden one'`).run()

    // Mirrors worldCorrectionsForWorldStmt in lib/db.ts.
    const visible = db
      .prepare(
        `SELECT player_text FROM world_corrections
         WHERE world_id = ? AND hidden = 0 ORDER BY id DESC`,
      )
      .all(1) as Array<{ player_text: string }>
    expect(visible.map((r) => r.player_text)).toEqual(['visible one'])

    // The row is withheld, not destroyed — the table stays append-only.
    const total = db
      .prepare(`SELECT COUNT(*) AS n FROM world_corrections WHERE world_id = 1`)
      .get() as { n: number }
...
expect(db.pragma('user_version', { simple: true })).toBe(41)
    ...
    const col = cols.find((c) => c.name === 'speech_register')
    expect(col?.type.toUpperCase()).toBe('TEXT')
    expect(col?.notnull).toBe(0)
    expect(col?.dflt_value).toBeNull()

    db.prepare(
      `UPDATE characters SET speech_register = ? WHERE world_id = 1 AND is_player = 1`,
    ).run('clipped · bureaucratic · never monologues')
    const row = db
      .prepare('SELECT speech_register FROM characters WHERE world_id = 1 AND is_player = 1')
      .get() as { speech_register: string }
    expect(row.speech_register).toContain('clipped')
  })
})

describe('v41 migration (character_refusals)', () => {
  it('adds a nullable refusals TEXT column to characters', () => {
    const db = seedV4Database()
    db.prepare(
      `INSERT INTO worlds (id, name, premise, initial_state_json) VALUES (?, ?, ?, ?)`,
    ).run(
    ...
    const col = cols.find((c) => c.name === 'refusals')
    expect(col?.type.toUpperCase()).toBe('TEXT')
    expect(col?.notnull).toBe(0)
    expect(col?.dflt_value).toBeNull()
  })
...
describe('v38 migration (world_events)', () => {
  it('creates world_events with world_id index', () => {
    const db = seedV4Database()
    db.prepare(
      `INSERT INTO worlds (id, name, premise, initial_state_json) VALUES (?, ?, ?, ?)`,
    ).run(
    ...
    const tables = db
      .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='world_events'")
      .all() as Array<{ name: string }>
    expect(tables).toHaveLength(1)

    const cols = db.prepare("PRAGMA table_info('world_events')").all() as Array<{
      name: string
    }>
    ...
    db.prepare(
      `INSERT INTO world_events (world_id, kind, source_agent, payload_json)
       VALUES (1, 'BEAT_DIRECTED', 'director', '{}')`,
    ).run()
    const row = db
      .prepare('SELECT kind, source_agent FROM world_events WHERE world_id = 1')
      .get() as { kind: string; source_agent: string }
    expect(row).toEqual({ kind: 'BEAT_DIRECTED', source_agent: 'director' })
  })
})

describe('v39 migration (director_state_json)', () => {
  it('adds a nullable director_state_json column on worlds', () => {
    const db = seedV4Database()
    db.prepare(
      `INSERT INTO worlds (id, name, premise, initial_state_json) VALUES (?, ?, ?, ?)`,
    ).run(
    ...
    runMigrations(db)
    expect(db.pragma('user_version', { simple: true })).toBe(41)
    const cols = db.prepare("PRAGMA table_info('worlds')").all() as Array<{ name: string }>
    expect(cols.some((c) => c.name === 'director_state_json')).toBe(true)
  })
})

describe('v40 migration (world_ui_skin)', () => {
  it('adds a nullable ui_skin column on worlds', () => {
    const db = seedV4Database()
    db.prepare(
      `INSERT INTO worlds (id, name, premise, initial_state_json) VALUES (?, ?, ?, ?)`,
    ).run(
    ...
    expect(db.pragma('user_version', { simple: true })).toBe(41)
    const cols = db.prepare("PRAGMA table_info('worlds')").all() as Array<{ name: string }>
    expect(cols.some((c) => c.name === 'ui_skin')).toBe(true)
  })
})
